...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('toan')
        .setDescription('Đếm ngược đến giờ thi môn Toán THPTQG 2025'),

    async execute(interaction) {
        const now = DateTime.now().setZone('Asia/Bangkok'); // GMT+7
        const target = DateTime.fromObject(
            { year: now.year, month: 6, day: 26, hour: 14, minute: 20 },
            { zone: 'Asia/Bangkok' }
        );
        ...
        const msg = `🧘 Thời gian còn lại đến khi thi môn Toán:\n` +
