...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('lichsu')
        .setDescription('Đếm ngược đến giờ thi môn Lịch Sử THPTQG 2025'),

    async execute(interaction) {
        const now = DateTime.now().setZone('Asia/Bangkok'); // GMT+7
        const target = DateTime.fromObject(
            { year: now.year, month: 6, day: 27, hour: 7, minute: 30 },
            { zone: 'Asia/Bangkok' }
        );
        ...
        const msg = `🧘 Thời gian còn lại đến khi thi môn Lịch Sử:\n` +
